...
var Overlay = {
    show: function(clickCallback, progressText)
    {
        var pageSize = getPageSize();
        var overlay = $("overlay");
...
function getProgressNode() {
	var div = Builder.node("div", { className:"progressbar" }, [ Builder.node("img", { src:habboStaticFilePath+"/images/progress_habbos.gif", /*width:"29", height:"6",*/ alt:"" }) ]);
	div.style.textAlign = "center";
	return Builder.node("p", [ div ]).innerHTML;
}

function setAsWaitDialog(a) {
    var b = $(a);
    if (b) {
        Element.wait($(b.id + "-body"))
    }
...
var ClientMessageHandler = {
    ...
    nielsen: function(data){
      ClientMessageHandler.url("https://web.archive.org/web/20130526105014/http://secure-dk.imrworldwide.com/cgi-bin/m?ci=Habbohotel&cg=0&si=" + data);
    },
