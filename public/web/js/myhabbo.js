...
function initView(id) {
    ...
    if (webStoreButton) {
        Event.observe(webStoreButton, "click", function (e) {
            Event.stop(e);
            loadWebStore(function () { if (window.WebStore) WebStore.open("webstore-store"); });
        });
    }
...
function openGroupActionDialog(confirmActionName, actionName, accountId, groupId, widget, onCompleteCallback) {
    ...
    new Ajax.Request(habboReqPath + confirmActionName, {
        method: "post", parameters: confirmParams,
...
MembersList.prototype = {
    ...
    confirm: function (actionName) {
        ...
        if (ids.length > 0) {
            ...
            new Ajax.Request(habboReqPath + "/myhabbo/groups/batch/confirm_" + actionName, {
                method: "post",
                ...
                onComplete: function (req, json) {
                    ...
                    if ($("group-action-ok")) {
                        Event.observe($("group-action-ok"), "click", function (e) {
                            Event.stop(e);
                            setDialogBody(dialog, getProgressNode());
                            new Ajax.Request(habboReqPath + "/myhabbo/groups/batch/" + actionName, {
                                method: "post",
...
function loadWebStore(onLoadCallback) {
    ScriptLoader.load("myhabbo-store", { callback: onLoadCallback || null });
}
