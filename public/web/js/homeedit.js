...
function editBg(D) {
    ...
    B.style.left = (E[0] - C[0] - A.width) + "px";
    B.style.top = (E[1] - C[1]) + "px";
...
function attachStickerObserver(A) {
    Event.observe("sticker-" + A + "-edit", "click", function(D) {
        ...
        B.value = "sticker-" + A;
        var E = $("dialog-edit-sticker");
        var C = $("sticker-" + A);
        E.style.top = C.style.top;
        E.style.left = C.style.left;
        bringElementOnTop(E);
        E.show()
    }, false)
}
...
var saveObserver = function(A) {
    ...
    if (showSaveOverlay()) {
		...
		console.log(_token);
        new Ajax.Updater("edit-save", habboReqPath + getSaveEditingActionName()+"?_token="+_token, {
            method: "post",
            evalScripts: true,
...
function openEditMenu(E, H, D, B, A) {
    Event.stop(E);
    closeEditMenu();
    var G = $(B).cumulativeOffset();
    var F = $("edit-menu");
    F.style.top = (G[1] - 5) + "px";
    F.style.left = (G[0] - 5) + "px";
    editMenuOpen = true;
    chosenElement = {
        id: H,
        type: D,
    ...
    if (D == "widget" || D == "stickie") {
        ...
        if (Element.hasClassName(C, "HighscoreListWidget")) {
            $("highscorelist-edit-menu").show();
            (WidgetRegistry.getWidgetById(H)).selectGameId()
        }
        if (D == "stickie") {
