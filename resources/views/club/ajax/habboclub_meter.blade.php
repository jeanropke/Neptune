...
@else
    @if (user()->subscription->neverSubscribed()) {{-- never been hc member --}}
    ...
    @elseif(user()->subscription->isExpired())
        ...
        <div id="subscription-meter-box2" style="left: 10px;">
            <div id="pastmonthsVal">{{ user()->subscription->getPassedMonths() }}</div>
            <div style="float: right; width: 160px; margin-right: 10px;">
                Seu Periodo
                de membro de {{ cms_config('hotel.name.short') }} Club venceu no dia
                {{ date('d/m/y', user()->subscription->expiresAt()) }}, aproveite e renove a
    ...
    @elseif (!user()->subscription->isExpired())
        {{-- has hc membership --}}
        @if (user()->subscription->daysRemaining() > 31)
